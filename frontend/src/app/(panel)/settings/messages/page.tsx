      ...
      <MotionCard delay={0} className="glass rounded-xl">
        <div className="px-5 py-4 border-b border-edge/50 section-header">
          <h3 className="text-sm font-semibold text-white flex items-center gap-2">
            <MessageSquare className="w-4 h-4 text-accent" /> Custom Messages
          </h3>
        ...
        <div className="p-5 space-y-5">
          {MESSAGE_KEYS.map(item => (
            <div key={item.key}>
              <label className="flex items-center gap-1.5 text-xs font-semibold text-white mb-1">
                <span className="w-1 h-1 rounded-full bg-accent/40" />{item.label}
              </label>
